...
#[async_trait]
impl Sampler for Zfs {
    type Statistic = ZfsStatistic;
    fn new(common: Common) -> Result<Self, anyhow::Error> {
        let fault_tolerant = common.config.general().fault_tolerant();
        let statistics = common.config().samplers().zfs().statistics();
    ...
    fn spawn(common: Common) {
        if common.config().samplers().zfs().enabled() {
            if let Ok(mut sampler) = Self::new(common.clone()) {
                ...
                fatal!("failed to initialize zfs sampler");
            } else {
                error!("failed to initialize zfs sampler");
            }
        }
    }
    ...
    fn sampler_config(&self) -> &dyn SamplerConfig<Statistic = Self::Statistic> {
        self.common.config().samplers().zfs()
    }
